...
import { db } from "./db";
import { z } from "zod";
import { evaluationVersion } from "./evaluation";
...
export async function registerJob(owner: string, input: unknown) {
  ...
  return db().begin(async (sql) => {
    ...
    for (const s of sessions) {
      ...
      const [reuse] = b.force
        ? []
        : await sql`SELECT i.result FROM atlas.job_items i JOIN atlas.jobs j ON j.id=i.job_id WHERE i.session_id=${s.id} AND i.revision=${s.revision} AND i.status='completed' AND i.expires_at>now() AND i.result->>'analysisVersion'=${version} AND (j.settings-'theme'-'timezone')=(${sql.json(settings)}::jsonb-'theme'-'timezone') ORDER BY j.created_at DESC LIMIT 1`;
