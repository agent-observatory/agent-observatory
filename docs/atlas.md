...
중성 회색·구분선·세션 표를 기본으로 한다. 상세 결과는 발견 사항 → 근거 → 개선 방향 순서이며, 근거 링크를 누르면 해당 이벤트로 이동한다.
