...
| 단계 | 다음 구현의 책임 |
|---|---|
| 수신 검증·마스킹 | 기존 서버 마스킹 경로에 크기를 제한한 압축 해제·계약 검증을 추가. 소유자 설정에 따른 마스킹 후 저장용 재압축·저장. 수동 업로드도 같은 경로를 사용하고 외부 AI에는 마스킹 정책이 적용된 근거만 전달 |
| 근거 정규화 | 원본 이벤트·턴·호출·결과를 연결. 같은 실행의 중복 표현만 통합하고 실제 재시도는 보존. 수동 업로드에도 같은 정책 적용 |
| 구간 구성 | 전송 배치와 독립적으로 턴·작업 흐름을 AI 입력 예산에 맞게 구성. 큰 턴은 하위 분할하고 경계 밖 선행 지시·미완료 호출을 참조로 연결 |
