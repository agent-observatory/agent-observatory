...
2026-09-12 제안이며 아래 복사 버튼·`guide`·Skill·MCP는 아직 구현하지 않았다.

| 우선순위 | 제공 방식 | 역할 |
| --- | --- | --- |
| 1 | 웹의 ‘에이전트용 프롬프트 복사’ | 설치·연결·수집 범위 설정이라는 작업 지시와 해당 작업에 필요한 가이드를 함께 복사 |
| 2 | npm 패키지에 버전별 가이드 동봉, `atlas-collector guide` | 설치된 CLI 버전과 맞는 명령·설정·전송 경계를 에이전트가 읽기 전용으로 확인 |
| 3 | 선택 설치하는 Skill | 에이전트가 Collector 작업을 발견하고 가이드·CLI를 필요할 때 호출하도록 연결 |
| 4 | 원격 MCP | 로그인한 사용자의 세션 조회·분석 실행·결과 가져오기를 도구로 제공 |

웹 프롬프트·패키지 가이드·Skill은 공통 명령 계약과 가이드 원본에서 생성한다. Skill은 내용을 복제하지 않고 가이드를 호출하는 짧은 절차로 유지한다. 웹은 최신 릴리스, 로컬은 설치 버전에 맞는 가이드를 제공한다. npm 패키지에 문서를 넣는 것만으로 에이전트가 자동으로 읽지는 않으며, 프롬프트나 Skill이 읽는 명령을 알려줘야 한다.

기본 프롬프트는 기존 설치·설정 확인 → 대상 프로젝트·기간과 예상 전송량 확인 → 사용자 요청 범위에 따른 연결·동기화 순서로 작성한다. 전체 세션 전송을 암묵적으로 지시하지 않는다. 설치 과정에서 사용자의 AGENTS.md·CLAUDE.md나 에이전트 전역 설정을 자동으로 수정하지 않는다. 원격 MCP와 로컬 파일·수집 설정 조작은 분리하며, 로컬 조작은 기존 CLI를 따른다.
