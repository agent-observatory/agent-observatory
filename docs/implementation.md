...
| 로컬 Collector | 설치·로컬 검증 | `~/.agent-session-atlas/current/cli.js`, LaunchAgent 하나·1,800초 간격, 일회용 코드 연결·macOS Keychain 키 저장 확인. 부분 줄·ACK 유실·Outbox 복구·잘못된 source 건너뛰기 테스트 등 6건 통과 |
...
| 제품 운영 배포 | 원격 READY 확인 | https://agent-session-atlas.vercel.app · Next.js 16.3.4 · Node 22 · 함수 `icn1` · 확인 배포 `dpl_7PVaDBLmvEKmSQr1pnXCV16uYDeV` |
| GitHub 검증 | 원격 검증 | [CI](https://github.com/agent-observatory/agent-session-atlas/actions/runs/34536817431), [유지관리 수동 실행](https://github.com/agent-observatory/agent-session-atlas/actions/runs/34536852830), [일일 분석 기동](https://github.com/agent-observatory/agent-session-atlas/actions/runs/34536855783) (`started: 1`), [Collector packaging](https://github.com/agent-observatory/agent-session-atlas/actions/runs/34536858875) 성공 |
| Collector Release | 원격 검증 | [collector-v0.1.0](https://github.com/agent-observatory/agent-session-atlas/releases/tag/collector-v0.1.0) 공개 및 tarball 설치 가능. npm registry 게시와 `npm whoami` 인증은 미확인 |
| 예약 작업 | 원격 dispatch 검증 | 수동 유지관리·일일 분석 dispatch 성공. 자연 schedule 실행은 아직 관찰하지 않음 |
